...
import { Plus, Package, ChevronRight, Clock, CheckCircle, XCircle, Search, Filter } from 'lucide-react'
...
export const metadata = {
  title: 'Mes Skills - Portail Développeur',
}
    ...
    <div className="py-8 md:py-12">
      <div className="container">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-3xl font-bold mb-1">Mes Skills</h1>
            <p className="text-muted-foreground">
              {skills.length} skill{skills.length !== 1 ? 's' : ''} • {skills.filter(s => s.status === 'published').length} publié{skills.filter(s => s.status === 'published').length !== 1 ? 's' : ''}
            </p>
          </div>
          <Button asChild>
          ...
          <Card>
            <CardContent className="py-12">
              <div className="text-center">
                ...
                <h3 className="text-lg font-semibold mb-2">Aucun skill</h3>
                <p className="text-muted-foreground mb-6 max-w-sm mx-auto">
                  Vous n&apos;avez pas encore créé de skill. Commencez par créer votre premier skill !
                </p>
                <Button asChild>
                  <Link href="/dev/skills/new">
                    <Plus className="h-4 w-4 mr-2" />
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-4 text-sm text-muted-foreground">
                          <span>{skill.versionsCount} version{skill.versionsCount > 1 ? 's' : ''}</span>
                          {skill.latestVersion && (
