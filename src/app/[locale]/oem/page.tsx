...
export async function generateMetadata() {
  const t = await getTranslations('oemPortal')
  return {
    title: t('title'),
  }
}
...
export default async function OemPortalPage() {
  ...
  if (!user) {
    redirect(`/${locale}/login?redirect=/oem`)
  }
  ...
  const stats = [
    { label: t('pendingReview'), value: pendingCount, icon: Clock, color: 'text-yellow-500' },
    { label: t('approved'), value: approvedCount, icon: CheckCircle, color: 'text-green-500' },
    { label: t('rejectedModifications'), value: rejectedCount, icon: XCircle, color: 'text-red-500' },
  ]
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-muted-foreground">{t('partnerPortal')}</p>
            </div>
          </div>
        </div>
        ...
        <Card className="mb-8">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {t('submissionsToReview')} ({pendingCount})
            </CardTitle>
            <CardDescription>{t('skillsAwaitingValidation')}</CardDescription>
          </CardHeader>
          <CardContent>
            {pendingSubmissions.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">{t('noPendingSubmission')}</div>
            ) : (
        ...
        {submissions.filter((s) => s.status !== 'oem_review').length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t('processedSubmissions')}</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {submissions
                    ...
                    return (
                      ...
                      >
                        ...
                        <Button variant="ghost" size="sm" asChild>
                          <Link href={`/oem/submissions/${submission.id}`}>{t('view')}</Link>
                        </Button>
                      </div>
                    )
                  })}
